...
加密套件: 用于在SSL / TLS握手期间协商安全设置的算法的组合。在ClientHello和ServerHello消息交换之后，客户端发送优先级列表的密码支持套件。然后，服务器使用从列表中选择的密码套件进行响应。
...
![alt](https://static001.geekbang.org/resource/image/9e/cf/9e99f797de30a15a11b0e4b4c8f810cf.png)
...
![alt](https://static001.geekbang.org/resource/image/d8/3b/d86648267d5504c7813b2d692620503b.png)

通过上图我们可以看出，HTTPS 首先要协商加解密方式，这个过程就是 HTTPS 建立安全连接的过程。为了让加密的密钥更加难以破解，我们让服务器和客户端同时决定密钥，具体过程如下：

1. 浏览器发送它所支持的加密套件列表和一个随机数 client-random，这里的`加密套件是指加密的方法`，加密套件列表就是指浏览器能支持多少种加密方法列表。
...
![alt](https://static001.geekbang.org/resource/image/b2/50/b2b893921491c62b29aaddc1d4fa9550.png)
...
![alt](https://static001.geekbang.org/resource/image/d5/45/d5cd34dbf3636ebc0e809aa424c53845.png)
...
接下来我们看看含有数字证书的 HTTPS 的请求流程

![alt](https://static001.geekbang.org/resource/image/77/af/77c852ff2202b2b7bb3299a96a0f4aaf.png)
...
不过我们并没有解释清楚如何通过数字证书来证明用户身份，所以接下来我们再来把这个问题解释清楚。
...
不过最后一步数字签名的过程还需要解释下：首先 CA 使用 `Hash 函数`来计算极客时间提交的明文信息，并得出信息摘要；然后 CA 再使用它的私钥对信息摘要进行加密，`加密后的密文就是 CA 颁给极客时间的数字签名`。这就相当于房管局在房产证上盖的章，这个章是可以去验证的，同样我们也可以通过数字签名来验证是否是该 CA 颁发的。
...
1、证书链
2、黑名单
3、顶级CA的证书，默认就在操作系统列表里了
4、自签名证书和CA证书的利弊
5、证书用法有很多种，U盾、个人、公司、网站、控件签名等
...
1. <https://time.geekbang.org/column/article/156181>
2. <https://freessl.cn/>
