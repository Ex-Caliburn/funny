...
果你想要学习一门新技术，最好的方式是先了解这门技术是如何诞生的，以及它所解决的问题是什么。了解了这些后，你才能抓住这门技术的本质。所以本文我们就来重点聊聊 JavaScript 引入 Promise 的动机，以及解决问题的几个核心关键点。
...
![alt](https://static001.geekbang.org/resource/image/01/85/01e40e30db7e8a91eb70ce02fd8a6985.png)
...
这段代码是先请求time.geekbang.org/?category，如果请求成功的话，那么再请求time.geekbang.org/column，如果再次请求成功的话，就继续请求time.geekbang.org。也就是说这段代码用了三层嵌套请求，就已经让代码变得混乱不堪，所以，我们还需要解决这种嵌套调用后混乱的代码结构。这段代码之所以看上去很乱，归结其原因有两点：
...
![alt](https://static001.geekbang.org/resource/image/ef/7f/efcc4fcbebe75b4f6e92c89b968b4a7f.png)

我的理解
then 中的resolve
没有return返回 会返回Promise.resolve(undefined)
有返回 是promise 直接返回，如果不是Promise, 将返回值用Promise.resolve()包装
...
是因为 Promise 对象的错误具有“冒泡”性质，会一直向后传递，直到被 onReject 函数处理或 catch 语句捕获为止。具备了这样“冒泡”的特性后，就不需要在每个 Promise 对象中单独捕获异常了。至于 Promise 错误的“冒泡”性质是怎么实现的

promise 体内的报错会被Promise.reject 封装，如果then的第二个reject函数没有设置，会一直返回传递 Promise.reject，知道有reject函数，或者catch方法，catch必须设置函数， 再往外是全局设置的 unhandledrejection
...
之所以出现这个错误，是由于 Bromise 的延迟绑定导致的，在调用到 onResolve_函数的时候，Bromise.then 还没有执行，所以执行上述代码的时候，当然会报“onResolve_ is not a function“的错误了。

也正是因为此，我们要改造 Bromise 中的 resolve 方法，让 resolve 延迟调用 onResolve_。

要让 resolve 中的 onResolve_函数延后执行，可以在 resolve 函数里面加上一个定时器，让其延时执行 onResolve_ 函数，你可以参考下面改造后的代码：
...
老师说的resolve 插入到微任务队列，当时可能是对的，但是现在是错的，
现在是等promise 状态变成fullfill ，将then，catch等 回调函数插入到微任务队列，所以和老师的解读不一样，这才是和代码运行一直，then，catch实现是一个链表结构，我的实现下意识模仿了链表

promise 的实现就是微任务
特点是 链式调用和透传，没有处理函数的 case，透传上一个 Promise 的 argument 和状态
...
我的理解
promise 体内的报错会被Promise.reject 封装，如果then的第二个reject函数没有设置，会一直返回传递 new Promise(reject) ,或者catch方法，catch必须设置回调函数， 再往外是全局设置的 unhandledrejection
