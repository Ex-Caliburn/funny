...
function extractDataByKeyword(text, keyword, contextLines = 3) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes(keyword)) {
      ...
      for (
        let j = Math.max(0, i - expandedContextLines);
        j < Math.min(lines.length, i + expandedContextLines + 1);
        j++
      ) {
      ...
      if (keyword.includes('外购') || keyword.includes('采购')) {
        ...
        if (match) {
          ...
          if (match) {
            ...
            if (
              !extractedValue &&
              !contextText.includes('采购集团') &&
              !contextText.includes('收购集团')
            ) {
              // 更精确的模式：外购后面跟着数字，然后是"万吨"，中间不能有"%"等符号
              purchasePattern = new RegExp(
                keyword + '[^%]*?([\\d,，]+\\.?\\d*)\\s*万吨',
                'g'
              )
              match = purchasePattern.exec(contextText)
      ...
      if (extractedValue) {
        const lineNumbers = line.match(patterns.number)
        const extractFunc = isWanTonKeyword ? extractNumberInWanTons : extractNumber
        const allValues = lineNumbers
          ? lineNumbers.map((n) => extractFunc(n)).filter((n) => n !== null)
          : [extractedValue]
...
function extractCoalFinancials(text) {
  ...
  const pattern1 =
    /煤炭销售收入[\s\t]+([\d,，]+\.?\d*)[\s\t]+亿元[，,]*[\s\t]*煤炭销售成本[\s\t]+([\d,，]+\.?\d*)[\s\t]+亿元[，,]*[\s\t]*毛利[\s\t]+([\d,，]+\.?\d*)[\s\t]*亿/g
  ...
  const pattern2 =
    /煤炭销售收入[\s\t]+([\d,，]+\.?\d*)[\s\t]+亿元[，,]*[\s\t]*煤炭销售成本[\s\t]+([\d,，]+\.?\d*)[\s\t]+亿元/g
...
function getMainValue(results, preferKeyword = null) {
  ...
  // 如果指定了优先关键词，先查找匹配的
  if (preferKeyword) {
    const preferredItems = results.filter(
      (r) => r.keyword && r.keyword.includes(preferKeyword)
    )
    if (preferredItems.length > 0) {
      // 从优先的结果中选择最大值
      const preferredValues = preferredItems
        .map((r) => r.value)
        .filter((v) => v !== null && v > 0)
...
function generateSummary(allData) {
  ...
  for (const data of allData) {
    ...
    for (const c of data.cost) {
      if (
        c.keyword === '煤炭销售成本' &&
        c.line.includes('从extractCoalFinancials提取')
      ) {
        coalCost = c.value
        break
      }
    ...
    if (purchase && production && Math.abs(purchase - production) < 10) {
      console.log(
        `  ⚠️  外购数据异常 (${purchase.toFixed(2)}万吨)，与产量数据(${production.toFixed(2)}万吨)相同，可能是误提取，已清空`
      )
      purchase = null
    ...
    if (coalCost && coalRevenue && Math.abs(coalCost - coalRevenue) < 1000000) {
      console.log(
        `  ⚠️  成本≈营收 (${(coalCost / 100000000).toFixed(2)}亿 ≈ ${(coalRevenue / 100000000).toFixed(2)}亿)，成本数据可能不准确`
      )
      // 不清空，但标记为可疑
    }
    ...
    if (coalRevenue && coalRevenue > 50000000000) {
      console.log(
        `  ⚠️  营收数据异常 (${(coalRevenue / 100000000).toFixed(2)}亿元)，可能是总营收而非煤炭营收`
      )
      // 不清空，但标记
    }
...
async function main() {
  ...
  summary.forEach((item) => {
    ...
    console.log(
      `  成本: ${item.cost ? (item.cost / 100000000).toFixed(2) + '亿元' : '未找到'}`
    )
    // purchase存储的是万吨，所以直接输出，不需要除以10000
    console.log(`  外购: ${item.purchase ? item.purchase.toFixed(2) + '万吨' : '未找到'}`)
    console.log(
      `  营收: ${item.revenue ? (item.revenue / 100000000).toFixed(2) + '亿元' : '未找到'}`
    )
    // 外采数据暂时不显示（所有数据都未找到）
    // console.log(`  外采: ${item.external ? (item.external / 100000000).toFixed(2) + '亿元' : '未找到'}`);
    console.log(
      `  产量: ${item.production ? item.production.toFixed(2) + '万吨' : '未找到'}`
    )
    console.log(`  销量: ${item.sales ? item.sales.toFixed(2) + '万吨' : '未找到'}`)
    console.log(
      `  库存: ${item.inventory ? item.inventory.toFixed(2) + '万吨' : '未找到'}`
    )
    if (item.productSales) {
      console.log(`  分产品销量:`)
      console.log(
        `    块煤: ${item.productSales.lumpCoal ? item.productSales.lumpCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    洗块煤: ${item.productSales.washedLumpCoal ? item.productSales.washedLumpCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    喷粉煤: ${item.productSales.pulverizedCoal ? item.productSales.pulverizedCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    洗粉煤: ${item.productSales.washedPulverizedCoal ? item.productSales.washedPulverizedCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    选末煤: ${item.productSales.selectedFineCoal ? item.productSales.selectedFineCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    洗末煤: ${item.productSales.washedFineCoal ? item.productSales.washedFineCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    末煤: ${item.productSales.fineCoal ? item.productSales.fineCoal.toFixed(2) + '万吨' : '未找到'}`
      )
      console.log(
        `    煤泥: ${item.productSales.slime ? item.productSales.slime.toFixed(2) + '万吨' : '未找到'}`
      )
    }
  })
...
function convertToNewFormat(rawItem) {
  ...
  if (coalData.revenue && coalData.sales && coalData.sales > 0) {
    coalData.unitPrice = parseFloat(
      ((coalData.revenue / coalData.sales) * 10000).toFixed(2)
    )
  }
  ...
  if (coalData.unitPrice && coalData.unitCost) {
    coalData.unitGrossProfit = parseFloat(
      (coalData.unitPrice - coalData.unitCost).toFixed(2)
    )
  }
  // 计算毛利率
 ...
 * 特点：
 * 1. 新报告期的数据会自动添加
 * 2. 已存在的报告期不会被自动流程修改（强保护，避免覆盖手动修正）
...
function generateCorrectedDataFile() {
  ...
  rawData.forEach((rawItem) => {
    ...
    if (!existingPeriods.has(period)) {
      ...
      // 已存在的报告期一律跳过，避免覆盖 corrected 文件中的手动修正
      skippedCount++
      console.log(`  ⏭ 跳过: ${period} (已存在，保护 corrected 数据)`)
    }
  })
...
function getPeriodFromFilename(filename, year) {
  ...
  if (filename.includes('第一季度')) {
    ...
    (filename.includes('年度报告') || filename.includes('年报')) &&
    !filename.includes('半年度')
  ) {
    return `${year}年全年`
  }
  return null
