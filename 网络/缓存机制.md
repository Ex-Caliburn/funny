---
topic: HTTP Caching Mechanism
category: Network
complexity: ⭐⭐⭐
tags: [StrongCaching, NegotiationCaching, CacheControl, ETag]
---
...
- 路由跳转不会刷新页面资源（SPA 特性），已打开会话不会自动获取新包。
- 可通过 WebSocket/SSE 推送“版本变更”事件，前端收到后提示用户刷新或自动执行 `location.reload()`。
- 也可轮询/心跳对比版本号/manifest，检测变更后再刷新。
- 保持 `index.html` 仅协商缓存；所有静态资源加 hash，避免旧包命中强缓存。
- 若使用 Service Worker，发布时配合 `skipWaiting` 与 `clients.claim`，并在激活后提示用户更新。
